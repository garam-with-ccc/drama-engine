...
__export(src_exports, {
  ...
  ModeratorDeputy: () => ModeratorDeputy,
...
var Prompter = class {
  ...
  constructor(config) {
    ...
    this.assemblePrompt = (companion, worldState, context, history, decorators = [], config = defaultPromptConfig, promptTemplate, returnChat = false) => {
      ...
      return this.renderPrompt(name, cleaned_chat, template ? new import_jinja.Template(template) : void 0);
    };
    this.template = new import_jinja.Template(config.chat_template);
    this.config = config;
  }
};
